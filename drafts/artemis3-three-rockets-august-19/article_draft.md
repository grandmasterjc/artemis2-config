---
id: artemis3-three-rockets-august-19
title: "Artemis 3 needs three rockets to fly in 2027, and NASA builds only one of them"
subtitle: "Jared Isaacman says he is extremely confident, and the Orion hardware at Kennedy backs him up. The harder question is the two commercial rockets that have to meet Orion in orbit, neither of which has yet done what the mission requires."
date: 2026-08-19
mission: artemis3
premium: false
hero_image: hero.jpg
push_title: "Artemis 3 in 2027 rests on three rockets"
push_body: "NASA's administrator says he is extremely confident. The SLS hardware supports him. The two commercial rockets Orion has to dock with are the open question."
---

Asked on Friday whether Artemis 3 would fly next year, NASA Administrator Jared Isaacman said he was "extremely confident" that it would. He was at Kennedy Space Center announcing an air and space expo alongside the center's director, Brian Hughes, and members of Florida's congressional delegation, and he pointed at hardware to explain the confidence. "That's why we stepped into stacking operations," he said, referring to the Space Launch System (SLS) rocket now going vertical in the Vehicle Assembly Building. NASA is managing "every single component" of the program, he added, "down to the day, down to the hour."

The hardware does support him, on the part NASA owns. Technicians joined Orion's crew and service modules on July 30 inside the Neil Armstrong Operations and Checkout Building. Four of the 10 solid rocket booster segments are stacked, engine integration is under way on the core stage, and cryogenic loading simulations were completed on August 5. A wet dress rehearsal, the full fueling test that precedes a launch campaign, is planned for November.

That is a real schedule with real hardware behind it. It also covers about a third of what Artemis 3 now requires. The mission Isaacman restructured in February is no longer a single launch. It is three launches on three different rockets, and NASA builds only one of them. The other two belong to Blue Origin and SpaceX, and as of this week neither has flown the thing Artemis 3 needs it to fly.

---

**Get the next briefing in your inbox. [Subscribe free →](https://artemis-briefing.kit.com)**

---

## What Artemis 3 turned into

On February 27, Isaacman announced that Artemis 3 would no longer attempt a crewed lunar landing. Instead it becomes a crewed test flight in low Earth orbit, where Orion practices the rendezvous and docking that Artemis 4 will have to perform at the moon, a landing currently set for no earlier than 2028. His reasoning at the time was straightforward. "What helps us get to the moon?" he said. "Well, for sure, rendezvous and docking with one or ideally both landers, that gives you an opportunity to do some integrated testing of a vehicle that we are going to depend upon the following year to take those astronauts down to the surface of the moon." He also put it more bluntly: "We've got to get back to basics."

The profile that emerged from that decision, as described by Spaceflight Now, runs like this. Blue Origin's New Glenn launches first, carrying a test article of the Blue Moon Mark 2 lander to low Earth orbit. SLS and Orion launch days later with commander Randy Bresnik and crewmates Luca Parmitano, Frank Rubio and Andre Douglas aboard, and rendezvous with it. Orion then docks a second time, with a Starship Version 3 carrying a docking adapter that launches on its own. The crew stays inside Orion throughout, because the Starship flying this mission has no habitable crew section.

Taking the landing out reduced the mission's ambition. It did not reduce the number of vehicles that have to work.

## Where New Glenn stands

New Glenn has flown three times with mixed results. The maiden flight in January 2025 reached orbit but lost its booster on landing, and a second flight that November went cleanly. On April 19 this year the rocket landed its booster on the recovery ship Jacklyn, a first for Blue Origin, then put its satellite in the wrong orbit.

Then, on May 28, the next New Glenn first stage exploded during a static fire test at Launch Complex 36 at Cape Canaveral. Blue Origin has since traced it to a specific part. "The anomaly originated at the main oxygen valve on one of the BE-4 engines, which was later confirmed by hardware recovery," chief executive Dave Limp said in early August. The company is modifying the valve in a way that can be retrofitted to engines already built, and Limp said the updated hardware would "be ready by the end of this month." Blue Origin is aiming to return New Glenn to flight before the end of 2026.

That fix has not flown yet. Neither has a Blue Moon lander: the uncrewed Mark 1, named Endurance, has slipped from summer 2026 to the first quarter of 2027 at the earliest, and it is waiting on the same rocket. The Mark 2 test article that Artemis 3 is supposed to dock with has never been to space.

![NASA illustration of Orion docked with a Starship human landing system. On Artemis 3 the docking happens in low Earth orbit, and the crew stays aboard Orion.](https://grandmasterjc.github.io/artemis2-config/updates/images/orion_hls_docking.jpg)

Both test articles, Blue Moon's and Starship's, have to fly and dock successfully before Orion can complete the rehearsal Isaacman is counting on.

## Where Starship stands

SpaceX's side of the mission has a different gap. Starship has flown 13 times over three years and has never completed an orbit. Every flight so far, including July's Flight 13, has been suborbital by design. Flight 14, targeted for late August, would be the first attempt at orbital insertion and the first attempt to catch a returning ship with the launch tower.

Isaacman named his own condition for confidence in SpaceX's contribution: the company needs to be launching Starship "every couple of weeks before the end of the year." That is far faster than the program has managed. Flight 12 flew on May 22 and Flight 13 on July 24, a gap of about nine weeks, and Flight 13 itself took three attempts across nine days after a pad abort and a weather slip. A two-week cadence would mean roughly quadrupling the current rate, on an orbital profile the vehicle has not flown before.

## How this compares with Apollo 9

Apollo 9 is the model Isaacman has reached for, and the comparison is apt in intent. In March 1969 a crew flew the command module and the lunar module together in Earth orbit, rehearsing the rendezvous and docking that Apollo 11 would perform at the moon that July. It was the step that made the landing credible.

Where the parallel thins out is the logistics. Apollo 9 was one launch. A single Saturn V carried both spacecraft, and once it cleared the pad every vehicle the mission depended on was already in orbit together. Artemis 3 spreads the same rehearsal across three launches run by NASA and two commercial companies, and both companies are still working through failures on the ground. Every additional launch is another schedule that has to hold and another vehicle that has to be ready in the same window. Isaacman's confidence rests on NASA managing its own component down to the hour, which the agency may well do, and on two companies hitting dates that neither has hit recently.

## What to watch

Flight 14, still expected late this month, is the nearest test, and the question is not only whether Starship reaches orbit but how quickly SpaceX turns the pad around afterward, since cadence is the part Isaacman put a number on. Next is New Glenn's return to flight, promised before year's end, which has to happen before any Blue Moon hardware reaches orbit. Then comes the November wet dress rehearsal, which will show whether the SLS and Orion side is holding its schedule. Watch also for NASA to name an actual target date for Artemis 3 rather than "next year," and for either company to say when its test article will be ready.

None of that makes Friday's answer wrong. The stacking is real, the Orion modules are joined, and the fueling test has a month attached to it. What Isaacman was asked about, though, was the whole mission, and the parts he cited are the parts NASA controls. The two he did not build have to arrive in orbit on time for any of it to count, and both are currently working their way back from something that broke.

If you want to follow the flights that will answer these questions as they happen, that is what our companion app Liftoff is built for. Live countdowns, real-time status and one-tap access to the official webcasts for every launch worldwide, free on the App Store: [Liftoff — Rocket & Space Launch](https://apps.apple.com/no/app/liftoff-rocket-space-launch/id6776392285).

Want more than the weekly briefing? The free email edition adds a short "Week Ahead" every Sunday, launch windows and milestones to watch, exclusive to email. [Subscribe to the Artemis Briefing →](https://artemis-briefing.kit.com)
